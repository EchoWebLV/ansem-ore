...
### M2c session keys (as-built) — realizes §4.2's `#[session_auth_or]` gate

Gasless, popup-free ER staking. A browser mints one **`SessionTokenV2`** on L1 (one wallet approval, via the gum session program `KeyspM2ssCJbqUhQ4k7sveSiY4WjnYsrXkC8oDbwde5`), then signs a burst of `stake` calls with an ephemeral key. `stake` is the **only** session-gated instruction: `#[derive(Accounts, Session)]` + `#[session_auth_or(miner.authority == authority, InvalidToken)]`. Session-signed ⇒ a valid `SessionTokenV2` (the gate recomputes the token PDA from `["session_token_v2", our_program, session_signer, authority]` and checks `now < valid_until` — so a token for another program, another wallet, another signer, or an expired one all fail); else the fallback ⇒ the signer must be the miner's wallet.

**Key wiring change:** `miner`/`escrow` seed on `miner.authority` (the stored wallet), **not** on `authority.key()` (the signer) — because when session-signed the signer *is* the ephemeral key, and the miner/escrow PDAs must still resolve to the wallet's. The signer↔owner binding moves from a seed constraint to the session gate.

**Containment (the [[avoid-rug-vector-test-seams]] concern):** `deposit`/`withdraw`/`claim` stay wallet-only, so a leaked session key can never move value **out** of escrow — max blast radius is one round's `max_stake_per_round`, and the token expires (≤ 7 days). Verified in tests: session-key `withdraw` is rejected. `top_up = false` (per §4.2): the ephemeral key stays at 0 lamports and is never the fee payer — on the ER a funded relayer/validator pays (gasless), on L1 the base wallet pays; the session key's only power is to *sign* `stake`. **Pin:** `session-keys =3.1.1` (feature `no-entrypoint`; wants `anchor-lang >=0.28,<2.0` → our 1.0.2 satisfies; not yanked → normal resolve). The gum program is **bundled by `mb-test-validator` at genesis** (verified) — no preload/clone, unlike the VRF pivot. TS: `@magicblock-labs/gum-sdk ^3.0.10` (`SessionTokenManager.createSessionV2`). **Local-testability note:** unlike in-ER VRF (§M2b, blocked locally), session keys test cleanly on the local stack because the token is a read-only cloned account the ER serves normally and the gum program is bundled — no delegated-queue requirement.
