...
- [ ] `solana-verify build --library-name ansem_miner` (Docker required — deploy THIS `.so`, never a local `anchor build`).
  - ⚠️ Task-2 finding: local agave CLI 4.1.0-beta.2 refuses old-arch `.so` at deploy (`cargo build-sbf --arch v3 --tools-version v1.54` was needed locally). At deploy time confirm (a) the solana-verify Docker build's `.so` is accepted by the deploy CLI, and (b) the on-chain hash matches what `verify-from-repo` reproduces — if the CLI arch-gate bites, pin an older agave for the deploy command rather than diverging the binary from the verifiable build.
